...
public class XMPPService extends GenericService {
	...
	private void createAdapter() {
		...
		mSmackable.registerCallback(new XMPPServiceCallback() {
			public void newMessage(String[] from, String message, boolean silent_notification, Type msgType, boolean isCarbon) {
				logInfo("new message: " + from +" with type: "+msgType.name());
				String name = (msgType == Type.groupchat) ? from[1] : mSmackable.getNameForJID(from[0]);
				notifyClient(from, name, message, !mIsBoundTo.contains(from), false, silent_notification, msgType, isCarbon);
			}
				...
				mMainHandler.post(new Runnable() {
					public void run() {
						...
						notifyClient(from, mSmackable.getNameForJID(from[0]), error,
							!mIsBoundTo.contains(from), silent_notification, true, Type.error, false);
					}});
				}
